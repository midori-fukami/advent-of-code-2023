...
namespace advent_of_code_2023
{
    public static class day_01
    {
        ...
        public static void part1()
        {
            ...
            {
                //it needs to replace the first one, not in the ordinal order, the order that appears...
                list[i] = list[i].Replace("one", "one1one").Replace("two", "two2two").Replace("three", "three3three")
                ...
                var lala = Regex.Match(list[i], @"(\d{1})").ToString();
                var lala2 = Regex.Match(list[i], @"(\d{1})", RegexOptions.RightToLeft).ToString();

                Console.WriteLine("first: " + lala + " last: " + lala2);
                sum += int.Parse(lala + lala2);
            }
